          ...
          <tbody className="divide-y divide-[#D8D2C4]/60">
            {filtered.map((f) => (
              <tr key={f.id} className="hover:bg-[#FBF9F5]">
                ...
                <td className="py-3.5 px-4 text-xs font-semibold text-[#555E68]">{f.designation}</td>
                <td className="py-3.5 px-4 text-xs text-[#555E68]">{f.department}</td>
                <td className="py-3.5 px-4 text-center font-mono text-xs font-bold text-[#12181F]">{f.lecturesConducted}</td>
                <td className="py-3.5 px-4 text-right">
                  <button onClick={() => onDeleteFaculty(f.id)} className="text-xs text-[#BA1A1A] hover:underline">
                    Delete
                  </button>
                </td>
              </tr>
