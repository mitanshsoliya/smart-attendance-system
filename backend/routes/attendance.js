...
router.post("/mark", verifyToken, requireStudent, validateAttendanceMark, (req, res) => {
  ...
  const processMarking = (lectureId) => {
    ...
    const studentSql = "SELECT id FROM students WHERE user_id = $1";
    ...
    db.query(studentSql, [req.user.id], (err, students) => {
      ...
      const proceedWithStudentId = (student_id) => {
        const checkSql = "SELECT id FROM attendance WHERE lecture_id = $1 AND student_id = $2";
        ...
        db.query(checkSql, [lectureId, student_id], (err, existing) => {
          ...
          const insertSql = "INSERT INTO attendance (lecture_id, student_id, status) VALUES ($1, $2, 'PRESENT')";
    ...
    db.query(
      ...
      (err, sessions) => {
        if (err) {
          ...
          return res.status(500).json({ message: "Database query error during QR verification." });
        }

        if (!sessions || sessions.length === 0) {
          return res.status(404).json({
            message: "Invalid QR code. Session not found in university records.",
          });
        }
        ...
        if (now > expiresAt) {
          return res.status(400).json({
            message: "QR session has expired. Please ask your instructor for an updated QR code.",
            expired: true,
            expiredAt: session.expires_at,
          });
        }

        processMarking(session.lecture_id);
      }
    );
  } else if (inputLectureId) {
    processMarking(Number(inputLectureId));
  } else {
